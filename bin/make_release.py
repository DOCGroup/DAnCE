...
def parse_args ():
    ...
    parser.add_option ("--root", dest="repo_root", action="store",
                       ...
                       default="https://github.com/DOCGroup/DAnCE.git")
...
def commit (files):
    ...
    version = "DAnCE-%d_%d_%d" % (comp_versions["DAnCE_major"],
                                 comp_versions["DAnCE_minor"],
                                 comp_versions["DAnCE_micro"])
...
def check_workspace ():
    """ Checks that the DAnCE repository is up to date.  """
    global opts, doc_root
    try:
        ex ("git pull -p")
        print "Successfully updated DAnCE working copy"
    except:
        print "Unable to update DAnCE workspace at " + doc_root
        raise
...
def create_changelog (component):
    ...
    old_tag = "DAnCE-%d_%d_%d" % (old_comp_versions["DAnCE_major"],
                                 old_comp_versions["DAnCE_minor"],
                                 old_comp_versions["DAnCE_micro"])
...
def tag ():
    """ Tags the DAnCE repository for the version and push that remote """
    global comp_versions, opts

    tagname = "DAnCE-%d_%d_%d" % (comp_versions["DAnCE_major"],
                                 comp_versions["DAnCE_minor"],
                                 comp_versions["DAnCE_micro"])
...
def push ():
    ...
    if opts.push:
        if opts.take_action:
            vprint ("Pushing DAnCE master to origin")
...
def export_wc (stage_dir):
    ...
    tag = "DAnCE-%d_%d_%d" % (comp_versions["DAnCE_major"],
                             comp_versions["DAnCE_minor"],
                             comp_versions["DAnCE_micro"])

    # Clone the DAnCE repository with the needed tag
...
def package (stage_dir, package_dir, decorator):
    """ Packages DAnCE releases of current
...
#        print "removing files", join (stage_dir, "zip-archive.zip"), join (stage_dir, "tar-archive.tar")
    ...
    text_files, bin_files = create_file_lists (join (stage_dir, "DAnCE"),
                                               "", [".gitignore", ".git"])
...
def create_kit ():
    ...
    version = "%d_%d_%d" % (comp_versions["DAnCE_major"],
                            comp_versions["DAnCE_minor"],
                            comp_versions["DAnCE_micro"])
